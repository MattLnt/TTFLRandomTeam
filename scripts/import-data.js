...
async function main() {
  const dataPath = path.join(__dirname, '../scraper/all_members.json')
  const raw = fs.readFileSync(dataPath, 'utf8')
  const data = JSON.parse(raw)

  console.log('🚀 Import des données TTFL...')

  for (const [pseudo, memberData] of Object.entries(data.members)) {
    console.log(`\n👤 Import de ${pseudo}...`)

    const totalScore = memberData.historique.reduce((sum, p) => {
